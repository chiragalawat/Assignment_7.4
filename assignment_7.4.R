...
library(foreach)
library(readr)
# [1] "C:\\Users\\CHIRAG\\Downloads\\ACADgILd"
setwd("C:\\Users\\CHIRAG\\Downloads\\ACADgILd\\BlogFeedback")

blogData_train <- read_csv("C:\\Users\\CHIRAG\\Downloads\\ACADgILd\\blogData_train.csv")
...
## [1] 0.397223
# try SVM
mdl_svm = svm(V281 ~ V52 + V55 + V61 + V51 + V54 + V21 + V6 + V10, data = tra
              in, kernel = "radial", cost = 2, gamma = 0.25)
pred_svm = predict(mdl_svm, test)
mse(pred_svm, test_y)
## [1] 0.4454636
# try XGboost
mdl_xgb = xgboost(data = train_xgb, nround = 750, nthread = 4, max_depth = 6,
                  eta = 0.025, subsample = 0.7, gamma = 3)
...
meta_xgb_train = foreach(i = 1:k, .combine = c) %do% {
  ...
  # xgb data
  train_set1_xgb = xgb.DMatrix(data = as.matrix(train_set1), label = train[tr
                                                                           ain_index]$V281)
  train_set2_xgb = xgb.DMatrix(data = as.matrix(train_set2), label = train[cv
                                                                           _index_split[[i]]]$V281)
  
  # level 0 prediction
  temp_xgb = xgboost(data = train_set1_xgb, nround = 750, nthread = 4, max_de
                     pth = 6, eta = 0.025, subsample = 0.7, gamma = 3)
...
train_sg = data.frame(meta_knn_train, meta_glm_train, meta_svm_train, meta_rf
                      _train, meta_xgb_train, train_y)
test_sg = data.frame(meta_knn_test, meta_glm_test, meta_svm_test, meta_rf_tes
...
mdl_glm = cv.glmnet(train_sg_sparse, train_y, family = "gaussian", alpha = 0.
                    2)
